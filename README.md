# pySame
**PySame** is an easy-to-use command line application which generates SAME _(Specific Area Message Encoding)_ headers, which are a basic building block of several civil defense warning systems, most notably the _Emergency Alert System_ in the United States. PySame will prompt you to enter all information which is carried in all real-life SAME headers - The event in question, the issue time of the message, the areas affected, etc, and generate a valid SAME header as a WAV audio file. Thanks to PySame, creators of simulated emergency messages can make them more realistic than ever!


_**Disclaimer:** The software (i.e. the PySame repository and all its contents) is provided for entertainment, educational and demonstrative purposes only, without warranty of any kind, either expressed or implied.
By using the software, you hereby consent that you will be solely responsible for any subsequent event or action taking place from such use, including but not limited to any usage of EAS tones generated by the software.
